...
class Database {
    ...
    _decodePacket(rawBuffer) {
        ...
        let packet = (0, _Protocol.Decode)(buffer);
    ...
    savePacket(scope, packet) {
        return this.client.evalAsync(PQADD, 1, `${this.namespace}:${scope}`, this.evalScore(packet), packet.identifier, this._encodePacket(packet));
    }

    unconfirmedPacket(scope, limit) {
        let now = (0, _moment2.default)().unix();
        return this.client.evalAsync(PQPOP, 1, `${this.namespace}:${scope}`, limit, now).then(function (buffers) {
            return _lodash2.default.map(buffers, function (buffer) {
